...
define(["sylv"], function() {
	...
	*/
	sylv.plot = function(id, distribution, x, options ,nop) {
	  
	  if (!( this instanceof arguments.callee )) {
		 return new sylv.plot(id, distribution,x, options,nop);
	   }
	...
	sylv.extend(sylv.plot, {
	  ...
	  setDistribution: function(distribution, x,obj) {
		  ...
		  if( distribution instanceof sylv.gamma || distribution instanceof sylv.lognormal) {
			  obj._min = 0 ;
		  }
		  if( distribution instanceof sylv.beta) {
			obj._min = 0.0;
			obj._max = 0.998999999;
		  }
		if(x == null)
			obj.x = sylv( sylv.seq( obj._min, obj._max, obj.nop) );
		else
			obj.x = x;
